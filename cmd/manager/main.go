...
func transportEnforcesInitialDependencies(transportType string) bool {
	switch transportType {
	case kafkaTransportTypeName:
		return false
...
func createManager(leaderElectionNamespace string, transportType string, workersPool *workerpool.DBWorkerPool,
	...
	transportEnforcesInitialDependencies := transportEnforcesInitialDependencies(transportType)
	conflationManager := conflator.NewConflationManager(ctrl.Log.WithName("conflation"), conflationReadyQueue,
		transportEnforcesInitialDependencies, statistics) // manage all Conflation Units
