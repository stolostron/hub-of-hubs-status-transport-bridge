	...
	for {
		...
		case <-ticker.C: // wait for next time interval
			...
			for Consumer := range c.consumers {
				...
				c.patchCommitMetadataMap(processedBundleMetadataToCommit, highestNonPendingMetadataPerPartition, false)
			}

			// patched the processed bundle metadata with that of the pending collection, so that if a partition
			// has both types, the pending bundle gains priority.
			...
			// DEBUGGING ONLY - DO NOT PUBLISH THIS CODE
			if len(processedBundleMetadataToCommit) > 0 {
				c.log.Info("attempted commit", "bundles metadata sent", *processedBundleMetadataToCommit[0])
			}
		}
	}
}
