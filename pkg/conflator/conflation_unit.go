...
func (cu *ConflationUnit) insert(bundle bundle.Bundle, metadata transport.BundleMetadata) {
	...
	if conflationElement.bundleInfo.GetMetadata() != nil && !conflationElement.isInProcess {
...
func (cu *ConflationUnit) checkDependency(conflationElement *conflationElement) bool {
	...
	// this allows transports that do not require an initial on-load generation check to pass.
	...
	case dependency.AtLeast:
		fallthrough // no-op case is AtLeast

	default:
		// dependencyLastProcessedVersion might be null, therefore order cannot change.
